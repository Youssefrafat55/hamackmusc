...
const youtube = new YouTube("AIzaSyAtTDjyu8pCazFbm7xx-pnhEzVdBYJKYrc"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyAtTDjyu8pCazFbm7xx-pnhEzVdBYJKYrc"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(` | The opening of the International Games `, { type: "STREAMING" });
});
  ...
  if (command == "say") {
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        "**ADMINISTRATOR You Don't have Permission :rolling_eyes:**"
      );
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "new")) {
    ...
    if (!message.guild.roles.exists(gg => gg.name === "Support Team"))
      return message.channel.send(`You need to create a role named \`Support Team\`.`);
    if (
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@599317636928634957>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  ...
  if (command == "ban") {
    ...
    if (!message.guild.member(message.author).hasPermission("BAN_MEMBERS"))
      return message.reply("**You Don't Have Ban Members Permission**");
...
client.on("message", message => {
  ...
  if (command == "kick") {
    ...
    let reason = message.content
      ...
      .slice(2)
      .join(" ");
    if (message.mentions.users.size < 1) return message.reply("**Mention some one**");
    if (!reason) return message.reply("**Write Kick Reason**");
    if (!message.guild.member(user).kickable)
      return message.reply(
        "**I Can't Kick This person check my premision**"
      );
    if (
      message.mentions.members.first().highestRole.position >=
      message.member.highestRole.position
    )
      return message.channel.send("Member Role is above you");
...
client.on("message", message => {
  ...
  if (command === prefix + "unmute") {
    ...
    if (!message.member.hasPermission("MANAGE_ROLES"))
      return message
        .reply("** You Don't Have premission 'Manage Roles' **")
        .catch(console.error);
    let user = message.mentions.users.first();
    ...
    if (!muteRole)
      return message
        .reply("** You Don't Have 'Muted' Role **")
        .catch(console.error);
    ...
    const embed = new Discord.RichEmbed()
      .setColor(0x00ae86)
      .setTimestamp()
      .addField("اUses:", "اسكت/احكي")
      .addField(
        ".. the person's mute has been Removed:",
    ...
    if (message.guild.member(user).removeRole(muteRole.id)) {
      return message
        .reply("**:white_check_mark: .. the person's mute has been Removed **")
        .catch(console.error);
    } else {
      message.guild
        .member(user)
...
client.on("message", message => {
  ...
  if (command === prefix + "mute") {
    ...
    if (message.mentions.users.size < 1)
      return message
        .reply("** You must mention member first**")
        .catch(console.error);
    ...
    if (message.guild.member(user).roles.has(muteRole.id)) {
      return message
        .reply("**:white_check_mark: .. Member has muted**")
        .catch(console.error);
    } else {
      message.guild
        .member(user)
...
client.on("message", message => {
  if (message.content === prefix + "close") {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.reply(" You Don't have premissions");
    message.channel
  ...
  if (message.content === prefix + "open") {
    if (!message.channel.guild)
      return message.reply(" You Don't have premission!!");
    ...
    message.channel
      ...
      .then(() => {
        message.reply("**Chat Un locked :white_check_mark:**");
      });
  }
});
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "giveaway")) {
    ...
    if (!message.guild.member(message.author).hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":heavy_multiplication_x:| **Must have premessins**"
      );
    message.channel
      .send(`** Wrte Room name ( # )**`)
      .then(msgg => {
  ...
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var embed = new Discord.RichEmbed()
        ...
        .setDescription(`**Good Buy ✋** `)
        .addField("👤   Left", `**[ ${member.guild.memberCount} ]**`, true)
        .setColor("RANDOM")
        .setFooter(`نتمنى لكم الاستمتاع`);

      var channel = member.guild.channels.find(gg => gg.name === "leave-igo"); //// تعديل اساسي
      if (!channel) return;
...
//// هذا البوت هو الاصدار الثاني من بوت السيستم by TARBOUN 
