...
setInterval(() => { // CODE BY KAHRBAA كههربا
  http.get(`http://hamaki3.glitch.me/`);
}, 280000);
...
const prefix = "0"; // البرافيكس
   const GUILDID = '725701567084363861'; // اي دي السيرفر  
   const CHANNELID = '726194169160269946'; // اي دي الروم
//======================================[Client]======================================
// CODE BY KAHRBAA كههربا

client.on('ready', () => {
  console.log(`Logged in as ${client.user.tag}!`);
   client.user.setActivity("YaSmeeN Up 🌙⚡",{type: 'WATCHING'})
});
// CODE BY KAHRBAA كههربا
const { Client } = require('discord.js');
const ytdl  = require('ytdl-core');// CODE BY KAHRBAA كههربا
const url = 'https://youtu.be/1y7HKqllOBg'; // هنا فديو الخاص ب القرأن الكريم كامل // CODE BY KAHRBAA كههربا


client.on('ready',async () => {
    console.log('تـم تشغيل الالبوم');
    // CODE BY KAHRBAA كههربا
    voiceStay(GUILDID, CHANNELID);
   function voiceStay(guildid, channelid) {
    if(!guildid) throw new Error('725701567084363861');
    if(!channelid) throw new Error('726194169160269946');
